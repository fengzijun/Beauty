...
namespace Beauty.Web.Page
{
    internal class PagerBuilder
    {
        ...
        private const string CopyrightText = "\r\n<!--ASP.NET MvcPager 1.3 for ASP.NET MVC 2.0 Copyright © 2009-2010 Webdiyer (http://en.webdiyer.com)-->\r\n";
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<li ><a >{0}</a></li>", item.Text));
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? String.Format("<li class='current'><a href='#'>{0}</a></li>", item.Text)
                                                 : String.Format("<li><a href='{0}'>{1}</a></li>", url, item.Text));
        }

        private MvcHtmlString GenerateJqAjaxPagerElement(PagerItem item)
        {
            if (item.Disabled)
                return CreateWrappedPagerElement(item, String.Format("<a >{0}</a>", item.Text));
